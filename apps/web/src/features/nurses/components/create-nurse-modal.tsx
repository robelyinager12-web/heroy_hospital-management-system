...
import { nursesApi } from "@/features/nurses/api/nurses.api";
...
export function CreateNurseModal({ open, onClose }: CreateNurseModalProps) {
  const queryClient = useQueryClient();
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", phone: "" });
  const [tempPassword, setTempPassword] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: () => nursesApi.create(form),
    onSuccess: (res: any) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white">Add Nurse</h2>
        ...
        {tempPassword ? (
          <div className="space-y-3">
            <p className="text-sm text-emerald-400">Nurse added successfully.</p>
            <p className="text-sm text-slate-300">
              Temporary password: <span className="font-mono text-white">{tempPassword}</span>
            </p>
            <p className="text-xs text-slate-500">Share this with the nurse so they can log in and set their own password.</p>
            <button
              onClick={handleClose}
          ...
          >
            ...
            >
              {mutation.isPending ? "Creating..." : "Add Nurse"}
            </button>
          </form>
        )}
      </div>
