...
import { emergencyApi } from "@/features/emergency/api/emergency.api";
...
export function CreateEmergencyModal({ open, onClose }: CreateEmergencyModalProps) {
  const queryClient = useQueryClient();
  const [form, setForm] = useState({ patientId: "", reason: "", notes: "" });

  const { data: patientsData } = useQuery({
    queryKey: ["patients", "for-emergency-select"],
    queryFn: async () => (await patientsApi.list({ page: 1 })).data as PatientListResponse,
    enabled: open,
  });

  const mutation = useMutation({
    mutationFn: () => emergencyApi.create(form),
    onSuccess: () => {
  ...
  function handleClose() {
    setForm({ patientId: "", reason: "", notes: "" });
    mutation.reset();
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border border-red-500/20 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white">New Emergency Case</h2>
