...
import { opdApi } from "@/features/opd/api/opd.api";
...
export function CheckinModal({ open, onClose }: CheckinModalProps) {
  ...
  const [form, setForm] = useState({ patientUserId: "", doctorUserId: "", reason: "", notes: "" });

  const { data: patientsData } = useQuery({
    queryKey: ["patients", "for-opd-select"],
    queryFn: async () => (await patientsApi.list({ page: 1 })).data as PatientListResponse,
    enabled: open,
  });

  const { data: doctorsData } = useQuery({
    queryKey: ["doctors", "for-opd-select"],
    queryFn: async () => (await doctorsApi.list({ page: 1 })).data as DoctorListResponse,
    enabled: open,
  });

  const mutation = useMutation({
    mutationFn: () => opdApi.create(form),
    onSuccess: () => {
  ...
  function handleClose() {
    setForm({ patientUserId: "", doctorUserId: "", reason: "", notes: "" });
    mutation.reset();
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white">OPD Check-In</h2>
        ...
        >
          ...
          >
            {mutation.isPending ? "Checking in..." : "Check In Patient"}
          </button>
        </form>
      </div>
    </div>
